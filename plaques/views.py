...
def my_account_view(request):
    # Instead of user=request.user, use the email of the logged-in user
    customer_orders = Order.objects.filter(email=request.user.email).order_by('-created_at')

    return render(request, 'my_account.html', {'orders': customer_orders})
